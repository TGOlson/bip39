...
testCases :: [TestCase]
testCases = parseTestCase <$> testData

parseTestCase :: [String] -> TestCase
parseTestCase = \case
  [hexStr, wordListStr, seedStr] -> TestCase hexStr (parseWordList wordListStr) seedStr
  x                              -> error $ "unexpected test case parse error on " <> show x

parseWordList :: String -> [BIP39Word]
parseWordList = fmap (read . uppercase) . words
  where
    uppercase []     = error "unexpected empty string in word list"
